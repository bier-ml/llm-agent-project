...
class StockPriceService:
    ...
    def get_stock_price_history(self, symbol: str, interval: str = "daily", outputsize: str = "compact"):
        logger.info(
            f"Fetching stock price history for {symbol} with interval: {interval}")
        ...
        try:
            ...
            if "Time Series (Daily)" in data or "Time Series (Intraday)" in data:
                ...
                df = df.astype(float)
                logger.info(
                    f"Successfully fetched stock price data for {symbol}")
                return df
            else:
                logger.error(
                    f"Error fetching stock price data: {data.get('Note', 'Unknown error')}")
                return pd.DataFrame()
        except Exception as e:
            logger.error(
                f"An error occurred while fetching stock price data: {e}")
            return pd.DataFrame()
